      ...
      <Navbar/>
        ...
        <div class="text-wrapper">
          ...
          <div class="user-info-social">
            <p class="followers"><span>{{user[2].followers}}</span> Followers</p>
            <p class="following"><span>{{this.user[3].following}}</span> Following</p>
            <p class="playlistnumber"><span>{{this.user[4].playlistNumber}}</span> Playlist</p>
          </div>
  <button class="logout" @click="logout">Logout</button>
        ...
        <div class="top-wrapper"> 
          <div class="user-top">
            ...
            <div v-for="(n,index) in 10" :key="index" class="user-track" :data-link="'https://open.spotify.com/album/'+topTracks.items[index].album.id" @click="openSpotify">
              <img class="top-img" :src="topTracks.items[index].album.images[2].url">
              <div class="track-info">
                <div class="track-title"><h5>{{topTracks.items[index].name}}</h5></div>
              <div class="track-album">
                <h5 v-for="artist in topTracks.items[index].artists" :key="artist">{{artist.name}}</h5>
                <h5>-</h5>
                <h5>{{topTracks.items[index].album.name}}</h5></div>
              </div>
            </div>
          </div>
          <div class="user-top">
            ...
            <div v-for="(n,index) in 10" :key="index" class="user-artist" :data-link="'https://open.spotify.com/album/'+topArtists.items[index].id" @click="openSpotify">
              <img class="top-img" :src="topArtists.items[index].images[2].url">
...
export default {
  ...
  methods:{
    ...
    async fetchUserInfo(){
      ...
      let getUserInfo=await fetch(this.API_URL+"/me",headers).then((response)=>{
        return response;
      }).then(response => {
      ...
      let getArtistFollow=await fetch(this.API_URL+"/me/following?type=artist",headers).then((response)=>{
        return response;
      }).then(response => {
          return response.json();
      }).then(response => {
      ...
      let getUserPlaylist=await fetch(this.API_URL+`/users/${getUserInfo.id}/playlists`,headers).then((response)=>{
        return response;
      }).then(response => {
          return response.json();
      }).then(response => {
    ...
    async fetchTopItems(){
     ...
     //Top Artists
      ...
      let topArtistsLongTerm = await fetch(this.API_URL+`/me/top/artists?limit=50&time_range=long_term`,headers).then((response)=>{
        return response;
      }).then(response => {
          return response.json();
      }).then(response => {
       ...
       let topArtistsMediumTerm = await fetch(this.API_URL+`/me/top/artists?limit=50`,headers).then((response)=>{
        return response;
      }).then(response => {
          return response.json();
      }).then(response => {
       ...
       let topArtistsShortTerm = await fetch(this.API_URL+`/me/top/artists?limit=50&time_range=short_term`,headers).then((response)=>{
        return response;
      }).then(response => {
          return response.json();
      }).then(response => {
    ...
    //Top Tracks 
      ...
      let topTracksLongTerm = await fetch(this.API_URL+`/me/top/tracks?limit=50&time_range=long_term`,headers).then((response)=>{
        return response;
      }).then(response => {
          return response.json();
      }).then(response => {
       ...
       let topTracksMediumTerm = await fetch(this.API_URL+`/me/top/tracks?limit=50`,headers).then((response)=>{
        return response;
      }).then(response => {
          return response.json();
      }).then(response => {
       ...
       let topTracksShortTerm = await fetch(this.API_URL+`/me/top/tracks?limit=50&time_range=short_term`,headers).then((response)=>{
        return response;
      }).then(response => {
          return response.json();
      }).then(response => {
...
<style scoped>
  ...
  .user-info-social>p:first-of-type,.user-info-social>p>span:first-of-type{
    margin-left: 0;
  }
  ...
  .user-track:hover ,.user-artist:hover{
    background: #4a1f54;
  }
  .user-artist{
    align-items: center;
  ...
  @media screen and (max-width: 1100px){
    ...
    .track-album h5,.user-info-social>p,.title-wrapper > button,.logout{
      font-size: 2vw;
    }
    .text-wrapper>h1{
      font-size: 5vw;
